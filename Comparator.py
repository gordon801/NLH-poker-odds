...
def find_straight(input):
    ...
    if len(straight_counter) == 5:
        straight_cards = [input[i] for i in card_order]
    ...
    if any(i >= 3 for i in indice_counter) and any(i >= 2 for i in indice_counter) and hand_strength > 2:
        for e, i in reversed(list(enumerate(indice_counter, start=1))):
            if i >= 3 and fh_trips == 0:
                ...
                fh_pair = e
        for i in cards_input:
            if i.getIndex() == fh_trips or i.getIndex() == fh_pair:
                fh_order.append(cards_input.index(i))

        print("Full House", "trips", fh_trips, "pair", fh_pair, fh_order)
